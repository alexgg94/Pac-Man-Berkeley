...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
      ...
      for a in gameState.getLegalActions(0):
        u = self.min_value(self.result(gameState, 0, a), 1, self.depth)
    ...
    def max_value(self, gameState, agent, depth):
      if self.terminalTest(gameState, depth): return self.utility(gameState)
      
      v = -sys.maxint
      for a in gameState.getLegalActions(agent):
        v = max(v, self.min_value(self.result(gameState, agent, a), 1, depth))
      return v
    ...
    def min_value(self, gameState, agent, depth):
      ...
      for a in gameState.getLegalActions(agent):
        if (agent == gameState.getNumAgents()-1):
          v = min(v, self.max_value(self.result(gameState, agent, a), 0, depth-1))
        else:
          v = min(v, self.min_value(self.result(gameState, agent, a), agent+1, depth))
      return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def min_value(self, gameState, agent, depth):
      ...
      for a in gameState.getLegalActions(agent):
        if (agent == gameState.getNumAgents()-1):
          v += self.max_value(self.result(gameState, agent, a), 0, depth-1)
        else:
          v += self.min_value(self.result(gameState, agent, a), agent+1, depth)
